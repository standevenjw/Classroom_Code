 ...
 ************************************/
 ...
 public class H2_TicTacToeC1
 {
	public static final int ROW = 3;// Number of rows in the grid
	public static final int COLUMN = 3;// Number of columns in the grid
	public static final int NUMPLAYER = 2;// The number of players
	...
	public static void main(String[] args)throws IOException
	{
		...
		int input, // Keyboard input 
			turn = 0;// Turn counter
		boolean result = false;// This stores when a game has been won or draw.
	...
	public static void displayGrid(int row, int column, String[] displayArray)
	{
		...
		for(C = 0; C < row * column - 1; C += column)// Grid with "row" number of rows
		{
			...
			{
				System.out.print(displayArray[C + I]);
				...
				if(I == column - 1 && C < column * (row - 1))// Add a row spacer to all but the last row
					...
					for(X = 0; X < column - 1; X++)
						System.out.print("----");// Each column needs 4 dashes
					System.out.print("---");// Last column needs 3 dashes 
					}
			}
		}
	...
	public static boolean resultTest(int row, int column, String[] move2, String[] player2, int numPlayer, int turn1)
	{
		...
		for(int testChar = 0; testChar < numPlayer && result1 == false; testChar++)// testChar represents the position in the player2 that holds a character
		{
			int C; // Loop index
